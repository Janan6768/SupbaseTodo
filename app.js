...
todoForm.addEventListener('submit', async (e) => {
  ...
  if (file) {
    ...
    if (uploadError) {
      todoMsg.textContent = 'Upload failed: ' + uploadError.message
...
function showToast(message, type = "success") {
  ...
  // reset classes
  toast.className =
    "fixed bottom-4 right-4 opacity-0 px-4 py-2 rounded text-white shadow-lg transition-opacity duration-500";

  // color based on type
  if (type === "success") toast.classList.add("bg-green-600");
  if (type === "error") toast.classList.add("bg-red-600");
  if (type === "info") toast.classList.add("bg-blue-600");
  if (type === "warning") toast.classList.add("bg-yellow-600");
